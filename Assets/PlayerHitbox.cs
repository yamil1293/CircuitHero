...
using System.Collections;
	...
	public class PlayerHitbox : MonoBehaviour {
		...
		[System.Serializable] public class DefaultHitbox {
		...
		[System.Serializable] public class CrouchHitbox {
		...
		[System.Serializable] public class JumpHitbox {
		...
		[HideInInspector] public string currentHitbox = "default";	// Set the current hitbox value.
		...
		private Controller2D playerPrefab;			// Get the Player class.
        ...
        playerPrefab = GetComponent<Controller2D>();
        ...
        if (transform.localScale.x > 0) {
            playerPrefab.transform.position = new Vector2 (pos.x + boxCollider.offset.x + (boxCollider.size.x / 2),
                pos.y + boxCollider.offset.y + (boxCollider.size.y / 2));
            playerPrefab.transform.position = new Vector2 (pos.x + boxCollider.offset.x + (boxCollider.size.x / 2) + 0.1f,
                pos.y + boxCollider.offset.y - (boxCollider.size.y / 2) - (circleCollider.radius/2));
			}
		...
		public void AllowedToStandUp() {
			...
			*/
			...
			// Set point A and point B to draw a rectangle. This is the difference between the default and crouching hitboxes.
			Vector2 pointA = new Vector2(pos.x + defaultHitbox.boxCenter.x - (defaultHitbox.boxSize.x / 2),
                pos.y + defaultHitbox.boxCenter.y + (defaultHitbox.boxSize.y / 2));
			Vector2 pointB = new Vector2(pos.x + crouchHitbox.boxCenter.x + (crouchHitbox.boxSize.x / 2),
                pos.y + crouchHitbox.boxCenter.y + (crouchHitbox.boxSize.y / 2));
		}
	}
